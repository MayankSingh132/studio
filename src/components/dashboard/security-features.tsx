        ...
        <div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                {features.map((feature) => (
                    <Card 
                        key={feature.title} 
                        className="flex flex-col transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:bg-accent hover:text-accent-foreground group"
                    >
                        <CardHeader>
                            <div className="flex items-center gap-3">
                                <feature.icon className="w-8 h-8 text-primary group-hover:text-accent-foreground" />
                                <CardTitle>{feature.title}</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="flex-1">
                            <p className="text-muted-foreground group-hover:text-accent-foreground/80">{feature.description}</p>
                        </CardContent>
                    </Card>
                ))}
            </div>
